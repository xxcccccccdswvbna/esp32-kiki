...
public:
    void setup() override {
        ...
        for (auto& dev : devices_) {
            subscribe("ct1/" + dev.id + "/set", &BLEGateway::on_mqtt_structured);
        }
        ...
        ESP_LOGI("BLEGW", "=================================");
        ESP_LOGI("BLEGW", "CT1 BLE Gateway %s", FIRMWARE_VERSION);
        ESP_LOGI("BLEGW", "Build: %s", FIRMWARE_BUILD_DATE);
        ESP_LOGI("BLEGW", "Devices: %d", (int)devices_.size());
        ESP_LOGI("BLEGW", "=================================");
    }
        ...
        switch (state_) {
            ...
            case WAIT_ACK:
                if (now - last_action_time_ >= 3000) {
                    ESP_LOGW("BLEGW", "Timeout %s", current_cmd_.device_id.c_str());
    ...
    void on_mqtt_structured(const std::string& topic, const std::string& payload) {
    ...
    void send_hex(const std::string& hex_str) {
        ...
        if (data.empty()) {
            ESP_LOGW("BLEGW", "Invalid hex");
            return;
        }
        send_raw(data);
        ESP_LOGI("BLEGW", "Raw send: %d bytes", (int)data.size());
    }
...
private:
    ...
    void enqueue(const std::string& device_id, const std::string& command, 
        ...
        for (auto& act : dev->actions) {
            if (act.name == action) {
                ...
                ESP_LOGI("BLEGW", "Queue %s -> %s", device_id.c_str(), action.c_str());
                return;
            }
        }
    ...
    void start_sending() {
        ...
        last_action_time_ = millis();
        ESP_LOGD("BLEGW", "Start %s -> %s", 
                 current_cmd_.device_id.c_str(), current_cmd_.action.c_str());
    }
